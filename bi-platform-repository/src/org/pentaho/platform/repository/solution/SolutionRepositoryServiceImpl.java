...
public class SolutionRepositoryServiceImpl implements ISolutionRepositoryService {
  ...
  public boolean createFolder(IPentahoSession userSession, String solution, String path, String name, String desc)
    ...
    if (parentSolutionFile != null && parentSolutionFile.isDirectory()) {
      ...
      repository
          .addSolutionFile(repositoryBaseURL, indexPath, ISolutionRepository.INDEX_FILENAME, defaultIndex.getBytes(), false);
      return true;
    }
    return false;
  }
  ...
  protected void processRepositoryFile(IPentahoSession session, boolean isAdministrator,
                                       ISolutionRepository repository, Node parentNode, ISolutionFile file,
      String[] filters) {
        ...
        try {
          ...
          if (child == null) {
            ...
            if (name.endsWith(".xaction")) { //$NON-NLS-1$
              ...
              final String paramServiceUrl =
                  contextPath +
                      "ServiceAction?solution=" + URLEncoder.encode(solution, URL_ENCODING) +//$NON-NLS-1$
                      "&path=" + URLEncoder.encode(path, URL_ENCODING) + //$NON-NLS-1$
                      "&action=" + URLEncoder.encode(name, URL_ENCODING) + //$NON-NLS-1$
                      "&component=xaction-parameter"; //$NON-NLS-1$
              child.setAttribute("param-service-url", paramServiceUrl);

              final String url =  contextPath +
                  "ViewAction?solution=" + URLEncoder.encode(solution, URL_ENCODING) + //$NON-NLS-1$
                  "&path=" + URLEncoder.encode(path, URL_ENCODING) + //$NON-NLS-1$
                  "&action=" + URLEncoder.encode(name, URL_ENCODING); //$NON-NLS-1$
              ...
              try {
                ...
                if (!"".equals(fileUrl)) { //$NON-NLS-1$
                  url = contextPath + fileUrl +
                      "?solution=" + URLEncoder.encode(solution, URL_ENCODING) +
                      "&path=" + URLEncoder.encode(path, URL_ENCODING) +
                      "&action=" + URLEncoder.encode(name, URL_ENCODING); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                } else {
                  ...
                  for (IPluginOperation operation : info.getOperations()) {
                    if (operation.getId().equalsIgnoreCase("RUN")) { //$NON-NLS-1$
                      ...
                      command = command.replaceAll("\\{solution\\}", URLEncoder.encode(solution, URL_ENCODING)); //$NON-NLS-1$
                      command = command.replaceAll("\\{path\\}", URLEncoder.encode(path, URL_ENCODING)); //$NON-NLS-1$
                      command = command.replaceAll("\\{name\\}", URLEncoder.encode(name, URL_ENCODING)); //$NON-NLS-1$
                  ...
                  if (url == null) {
                    url = contextPath
                      + "content/" + handlerId +
                        "?solution=" + URLEncoder.encode(solution, URL_ENCODING) +
                        "&path=" + URLEncoder.encode(path, URL_ENCODING) +
                        "&action=" + URLEncoder.encode(name, URL_ENCODING); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
                  }
                }
  ...
  protected class PluginFileInputStream extends InputStream {
    ...
    {
      if (inputStream == null) {
        inputStream = repository.getResourceInputStream(relativePath, true, ISolutionRepository.ACTION_EXECUTE);
      }
