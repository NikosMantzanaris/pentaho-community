...
public class KettleComponent extends ComponentBase implements RowListener {
  ...
  private static final ArrayList<String> outputParams = new ArrayList<String>(Arrays.asList(EXECUTION_STATUS_OUTPUT, EXECUTION_LOG_OUTPUT,
      TRANSFORM_SUCCESS_OUTPUT, TRANSFORM_ERROR_OUTPUT, TRANSFORM_SUCCESS_COUNT_OUTPUT, TRANSFORM_ERROR_COUNT_OUTPUT));
    ...
    try {
      if (isDefinedInput(KettleComponent.DIRECTORY)) {
        ...
        try {
          ...
          error(Messages.getErrorString("Kettle.ERROR_0015_BAD_RESOURCE", KettleComponent.TRANSFORMFILE, fileAddress), e); //$NON-NLS-1$
          return false;
        }
        ...
        if (transMeta == null) {
          ...
          transMeta.setArguments(arguments);
          /*
           * We do not need to concatenate the solutionPath info as the fileAddress has the complete location of the file from start to end. This is to resolve
           * BISERVER-502.
           */
          transMeta.setFilename(fileAddress);
        }
  ...
  private TransMeta loadTransformFromRepository(final String directoryName, final String transformationName, final Repository repository,
    ...
    try {
      ...
      try {
        ...
        error(Messages.getErrorString("Kettle.ERROR_0009_TRANSFROMATION_METADATA_NOT_FOUND", directoryName + "/" + transformationName), e); //$NON-NLS-1$ //$NON-NLS-2$
        return null;
      }
      if (transMeta == null) {
        error(Messages.getErrorString("Kettle.ERROR_0009_TRANSFROMATION_METADATA_NOT_FOUND", directoryName + "/" + transformationName)); //$NON-NLS-1$ //$NON-NLS-2$
        debug(kettleUserAppender.getBuffer().toString());
        return null;
      } else {
        return transMeta;
  ...
  private JobMeta loadJobFromRepository(final String directoryName, final String jobName, final Repository repository, final LogWriter logWriter) {
    if (ComponentBase.debug) {
      debug(Messages.getString("Kettle.DEBUG_DIRECTORY", directoryName)); //$NON-NLS-1$
    }
    if (ComponentBase.debug) {
    ...
    try {
      ...
      try {
        ...
        error(Messages.getErrorString("Kettle.ERROR_0020_JOB_METADATA_NOT_FOUND", directoryName + "/" + jobName), e); //$NON-NLS-1$ //$NON-NLS-2$
        return null;
      }
      if (jobMeta == null) {
        error(Messages.getErrorString("Kettle.ERROR_0020_JOB_METADATA_NOT_FOUND", directoryName + "/" + jobName)); //$NON-NLS-1$ //$NON-NLS-2$
        debug(kettleUserAppender.getBuffer().toString());
        return null;
      } else {
        return jobMeta;
