...
public class DefaultChartBeansGenerator implements IChartBeansGenerator {
  ...
  private static final String DEFAULT_HTML_TEMPLATE = "<html><head><title>Command: doChart</title>{0}</head><body style='margin:0;' scroll='no'>{1}</body></html>"; //$NON-NLS-1$

  private static final String JFREE_CHART_HTML_TEMPLATE = "<html><head><title>Pentaho Chart</title></head><body style='margin:0;' scroll='no'>{imageMap}<img border=\"0\" width=\"{chartWidth}\" height=\"{chartHeight}\" src=\"{contextPath}getImage?image={imageFile}\" {useMap}/></body></html>"; //$NON-NLS-1$
  
  private static String DEFAULT_flashScriptFragment = "<input type=\"hidden\"/><script defer='defer' type=\"text/javascript\">function {dataFunction}() { return /*JSON*/\"{chartJson}\"/*END_JSON*/;}</script>"; //$NON-NLS-1$
  ...
  private InputStream internalCreateChart(IPentahoSession pentahoSession, Map<String, Object> parameterMap, String serializedChartDataDefinition,
      String serializedChartModel, int chartWidth, int chartHeight, String contentLinkingTemplate, OutputStream outputStream) throws IOException {
  ...
  protected void createAndRunActionSequence(final IPentahoSession pentahoSession,
      final Map<String, Object> params, final Map<String, Object> defaultParameterMap, String contentLinkingTemplate, final OutputStream out) {
  ...
  protected ActionSequenceDocument createActionSequenceDocument(final Set<String> parameterNameSet, String contentLinkingTemplate) {
    ...
    if(contentLinkingTemplate != null) pojoAction.setComponentDefinition("contentLinkingTemplate", contentLinkingTemplate);
  ...
  public String createChartAsHtml(IPentahoSession userSession, Map<String, Object> parameterMap,
      String serializedChartDataDefinition, String serializedChartModel, int chartWidth, int chartHeight, String contentLinkingTemplate)
      throws IOException {
    ...
    if(contentLinkingTemplate == null){//use old version
      ...
      InputStream is = this.internalCreateChart(userSession, parameterMap, serializedChartDataDefinition, serializedChartModel,
