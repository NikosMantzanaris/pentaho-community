...
public class FileItem extends FlexTable implements HasAllMouseHandlers, IFileSummary {
  ...
  public void handleRightClick(final int left, final int top) {
    ...
    for (int i = 0; i < menuItems.length; i++) {
      ...
      if (menuItems[i].equals(SEPARATOR)) {
        ...
        menuBar.addItem(new MenuItem(Messages.getString(menuItems[i]), new FileCommand(menuCommands[i], popupMenu, this)));
      } else {
  ...
  public FileItem makeDragProxy(){
    FileItem f = new FileItem(getName(), getLocalizedName(), "", getSolution(), getPath(), "", getURL(),
        filesListPanel, options, false, getIcon());
    f.enableDrag();
    return f;
  }
  ...
  public void enableDrag(){
    ...
    addStyleName("fileItemDragProxy");
    setDroppable(false);
  ...
  public void setDroppable(boolean canDrop){
    ...
    if(canDrop){
      ...
      addStyleName("validDrop");
    } else {
      MantleImages.images.drop_invalid().applyTo(dropIndicator);
      removeStyleName("validDrop");
    }
