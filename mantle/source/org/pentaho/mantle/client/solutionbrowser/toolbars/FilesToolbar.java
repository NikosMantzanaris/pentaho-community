...
public class FilesToolbar extends Toolbar implements IFileItemListener {
  ...
  private void createMenus() {
    ...
    runCmd = new FileCommand(FileCommand.COMMAND.RUN, null, fileSummaryProvider);
    ...
    editCmd = new FileCommand(FileCommand.COMMAND.EDIT, null, fileSummaryProvider);
  ...
  private void createMenuItems(final boolean supportsACLs) {
    ...
    for (int i = 0; i < menuCommands.length; i++) {
      ...
      if (menuCommands[i] == null) {
        ...
        menuFileCommands[i] = new FileCommand(menuCommands[i], miscComboBtn.getPopup(), fileSummaryProvider);
