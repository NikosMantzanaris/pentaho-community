...
public class PluginOptionsHelper {
  ...
  public static class ContentTypePlugin {
    ...
    private String replacePattern(String url, FileItem item) {
      ...
      String newurl = url.replaceAll("\\{solution\\}", URL.encodeComponent(item.getSolution())); //$NON-NLS-1$
      newurl = newurl.replaceAll("\\{path\\}", URL.encodeComponent(item.getPath())); //$NON-NLS-1$
      newurl = newurl.replaceAll("\\{name\\}", URL.encodeComponent(item.getName())); //$NON-NLS-1$
      newurl = newurl.replaceAll("\\{locale\\}", localeName); //$NON-NLS-1$
      return newurl;
    }
