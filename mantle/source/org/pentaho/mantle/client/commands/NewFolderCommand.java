...
public class NewFolderCommand extends AbstractCommand {
  ...
  protected void performOperation(boolean feedback) {
    if (fileSummary == null) { // request to create folder at root
      repoPath = "";
      solution = "";
    } else { // otherwise, fileSummary points to parent folder
      String tmpPath;
      if (fileSummary.getSolution().equals(fileSummary.getName())) {
        tmpPath = "";
      } else {
        tmpPath = "".equals(fileSummary.getPath()) ? fileSummary.getName() : fileSummary.getPath() + "/" + fileSummary.getName();
      }
      repoPath = tmpPath;
      solution = fileSummary.getSolution();
    }
    ...
    final IDialogCallback callback = new IDialogCallback() {
      ...
      public void okPressed() {
        ...
        if (GWT.isScript()) {
          ...
          // Add fixes for BISERVER-6918 - encode all bits of the URL because the doc for
          // RequestBuilder makes it clear that the URL should be pre-encoded.
          url = windowpath + "SolutionRepositoryService?component=createNewFolder&solution=" + //$NON-NLS-1$ 
              URL.encodeComponent(solution) + "&path=" + //$NON-NLS-1$
              URL.encodeComponent(repoPath) + "&name=" + //$NON-NLS-1$
              URL.encodeComponent(folderNameTextBox.getText()); //$NON-NLS-1$
