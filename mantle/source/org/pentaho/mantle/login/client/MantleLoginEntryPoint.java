  ...
  public void bundleLoaded(String bundleName) {
    AsyncCallback<Boolean> callback = new AsyncCallback<Boolean>() {
      ...
      public void onSuccess(Boolean newWindow) {
        if (newWindow) {
          String URL = (!StringUtils.isEmpty(returnLocation)) ? returnLocation.replace("Login", "Home") : Window.Location.getPath().replace("Login", "Home"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
