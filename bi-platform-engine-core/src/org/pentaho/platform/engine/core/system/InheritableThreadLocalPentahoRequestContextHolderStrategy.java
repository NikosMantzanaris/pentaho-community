...
public class InheritableThreadLocalPentahoRequestContextHolderStrategy implements IPentahoRequestContextHolderStrategy {
   ...
   * It is important that the framework calls this to prevent request context bleed-
  ...
  public void removeRequestContext() {
    ...
    if (requestContext != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(Messages.getString("PentahoRequestContextHolder.DEBUG_REMOVING_REQUEST_CONTEXT",//$NON-NLS-1$
            Thread.currentThread().getName(), String.valueOf(Thread.currentThread().getId())));
      }
      if (logger.isTraceEnabled()) {
        StackTraceElement[] elements = Thread.currentThread().getStackTrace();
        logger.trace(Messages.getString("PentahoRequestContextHolder.DEBUG_THREAD_STACK_TRACE"));//$NON-NLS-1$
