 ...
 *
 * Copyright 2005 - 2012 Pentaho Corporation.  All rights reserved.
 *
 */
...
public class LocalizationServlet extends ServletBase {
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    try {
      ...
      error(Messages.getErrorString("LocalizationServlet.ERROR_0000_ERROR"), ex); //$NON-NLS-1$
      resp.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
    }
  }

  /**
   * Retrieve a {@link ResourceBundle} from a plugin.
   *
   * @param pluginId ID of the plugin to load the resource bundle from
   * @param name     Resource bundle name that resides in the plugin
   * @return Resource bundle for the name provided in the plugin referenced by {@code pluginId}
   * @throws IllegalArgumentException Invalid plugin Id
   * @throws java.util.MissingResourceException Invalid resource bundle name
   */
  protected ResourceBundle getBundle(String pluginId, String name) {
    ...
    if (pluginClassLoader == null) {
      throw new IllegalArgumentException(Messages.getErrorString("LocalizationServlet.ERROR_0001_INVALID_PLUGIN_ID", pluginId)); //$NON-NLS-1$
    }
    if (name == null || name.length() == 0) {
      throw new IllegalArgumentException(Messages.getErrorString("LocalizationServlet.ERROR_0002_INVALID_RESOURCE_NAME", name)); //$NON-NLS-1$
    }
   ...
   * @param pluginId ID of plugin whose "cache-messages" setting should be checked
   ...
   * (http://dojotoolkit.org/reference-guide/1.7/quickstart/internationalization/index.html#quickstart-internationalization-index)
   *
   * @param pluginId ID of plugin to load the resource from
   ...
   *
   * @param bundle Resource bundle to convert
