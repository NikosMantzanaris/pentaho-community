...
public class HttpWebService extends ServletBase {
      ...
      throws IOException
  {
    ...
    {
      logger.debug(Messages.getString("HttpWebService.ERROR_0002_NOTFOUND", solutionName, actionPath, actionName));
    ...
    try
    {
      ...
      createParameterElement(parametersElement,
          "solution", String.class, null, "system", "system", new String[]{solutionName});
      createParameterElement(parametersElement,
          "path", String.class, null, "system", "system", new String[]{actionPath});
      createParameterElement(parametersElement,
          "action", String.class, null, "system", "system", new String[]{actionName});
      createParameterElement(parametersElement,
          "prompt", String.class, null, "system", "system", new String[]{"yes", "no"});
      createParameterElement(parametersElement,
          "instance-id", String.class, null, "system", "system",
          new String[]{parameterProvider.getStringParameter("instance-id", null)});

    }
    catch (Exception e)
    {
      logger.warn(Messages.getString("HttpWebService.ERROR_0003_UNEXPECTED"), e);
      response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
      return;
    }
  ...
  private Element createParameterElement (final Element parametersElement,
    ...
    if (service != null) {
      ...
      for (Iterator usersIterator = users.iterator(); usersIterator.hasNext();) {
        ...
        if ((null != username) && (username.length() > 0)) {
          buf.append("<user><![CDATA[" + username + "]]></user>"); //$NON-NLS-1$ //$NON-NLS-2$
        }
      }
    }
    buf.append("</users>"); //$NON-NLS-1$
    ...
    if (service != null) {
      ...
      for (Iterator rolesIterator = roles.iterator(); rolesIterator.hasNext();) {
        ...
        if ((null != roleName) && (roleName.length() > 0)) {
          buf.append("<role><![CDATA[" + roleName + "]]></role>"); //$NON-NLS-1$ //$NON-NLS-2$
        }
      }
    }
    buf.append("</roles>"); //$NON-NLS-1$
    ...
    if (validPermissionsNameMap != null) {
      ...
      for (Iterator aclsIterator = aclsKeySet.iterator(); aclsIterator.hasNext();) {
        ...
        if ((null != aclName) && (aclName.length() > 0) && (null != aclMask) && (aclMask.length() > 0)) {
          ...
          buf.append("<name><![CDATA["); //$NON-NLS-1$
          buf.append(aclName);
