...
public class SolutionContextListener implements ServletContextListener {
  ...
  public void contextInitialized(final ServletContextEvent event) {
    ...
    IApplicationContext applicationContext = new WebApplicationContext(SolutionContextListener.solutionPath, fullyQualifiedServerUrl,
    ...
    if( PentahoSystem.getObjectFactory().objectDefined( IVersionHelper.class.getSimpleName() ) ) {
      ...
      if (initOk) {
        System.out
            .println(Messages
                .getString(
                    "SolutionContextListener.INFO_SYSTEM_READY", "(" + helper.getVersionInformation(PentahoSystem.class) + ")", fullyQualifiedServerUrl, SolutionContextListener.solutionPath)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      } else {
        System.err
            .println(Messages
                .getString(
                    "SolutionContextListener.INFO_SYSTEM_NOT_READY", "(" + helper.getVersionInformation(PentahoSystem.class) + ")", fullyQualifiedServerUrl, SolutionContextListener.solutionPath)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      }
    }
  }
