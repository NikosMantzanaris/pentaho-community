...
/**
 * This class provides the foundation for combining a users' tenant ID with the
 * datasource name being requested at runtime. The concept is this - - -
 * 
...
public abstract class AbstractTenantAwareDatasourceService extends PooledOrJndiDatasourceService implements InitializingBean  {
  ...
  private String datasourceNameFormat = "{0}-{1}"; // {0} is always the tenant ID, {1} is always the requested datasource.
  ...
  public DataSource getDataSource(String dsName) throws DatasourceServiceException {
    ...
    if (tenantId == null) { // If tenant ID is null, what should it do?
      ...
      return super.getDataSource(MessageFormat.format(getDatasourceNameFormat(), tenantId, dsName)); // Get the tenant-ds instead
    } 
  }
