  ...
  public Connection getAuditConnection() throws SQLException {
    ...
    for (int i = 0; i <= 3; i++) {
      ...
      try {
        ...
        sqlEx = ex;
        AuditConnection.logger.warn(Messages.getErrorString("AuditConnection.WARN_0001_CONNECTION_ATTEMPT_FAILED", "" //$NON-NLS-1$ //$NON-NLS-2$
            + sleepTime[i]));
      }
    }
    throw new SQLException(Messages.getErrorString("AUDSQLENT.ERROR_0001_INVALID_CONNECTION"), sqlEx); //$NON-NLS-1$
  }
