...
public class DefaultPluginManager extends AbstractPluginManager {
  ...
  private void registerContentTypes(IPlatformPlugin plugin, ClassLoader loader)
    ...
    for (IContentInfo info : plugin.getContentInfos()) {
      ...
      if (!StringUtils.isEmpty(metaProviderClass)) {
        ...
        objectFactory.defineObject(METAPROVIDER_KEY_PREFIX+info.getExtension(), metaProviderClass, Scope.LOCAL, loader);
        ...
        try {
          ...
          objectFactory.get(Object.class, METAPROVIDER_KEY_PREFIX+info.getExtension(), null); //solution file meta providers cannot be session scoped, so null is ok here
        } catch (Exception e) {
          throw new PlatformPluginRegistrationException(defaultErrMsg, e);
        }
      }
  ...
  private ClassLoader setPluginClassLoader(IPlatformPlugin plugin) throws PlatformPluginRegistrationException {
    ...
    if (loader == null) {
      ...
      if(!pluginDir.exists() || !pluginDir.isDirectory() || !pluginDir.canRead()) {
        throw new PlatformPluginRegistrationException(Messages.getErrorString("PluginManager.ERROR_0027_PLUGIN_DIR_UNAVAILABLE", pluginDir.getAbsolutePath())); //$NON-NLS-1$
      }
  ...
  public IFileInfo getFileInfo(String extension, IPentahoSession session, ISolutionFile solutionFile, InputStream in) {
    ...
    try {
      ...
      if (objectFactory.objectDefined(objectKey)) {
        metaProvider = objectFactory.get(Object.class, objectKey, null); //session scope not supported for meta providers
      }
      if (metaProvider instanceof ISolutionFileMetaProvider) {
        ...
        // old nasty stuff
        fileInfo = getLegacyFileInfo((IFileInfoGenerator)metaProvider, session, solutionFile);
      }
    } catch (Throwable t) {
