...
public class PluginClassLoader extends URLClassLoader {
  ...
  /*
   * Backward Compatibility methods -- these will be removed in the next minor release
   */
  @Deprecated
  ...
  public PluginClassLoader(final String pluginDir, Object parent) {
    this(new File(pluginDir.replace('\\', '/')), parent);
  }
  
  @Deprecated
  public boolean isPluginClass(Class<?> clazz) {
    Class<?> t = findLoadedClass(clazz.getName());
    return (t != null);
  }
  /*
  ...
  public Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {
    ...
    Class<?> t = null;
    /*
     * Here we check the cache to see if the class has been loaded before by 
     * either this or the parent classloader.  If it has, then there is no need to load
     * the class again, just return it.
     */
    ...
    /*
     * If we are overriding the parent classloader we will first try to load this class from
     * this classloader object in isolation (no awareness of the parent classloader).  If 
     * this classloader does not have the class, we will proceed to attempt to load the class 
     * from the parent.
     */
    if (overrideLoad) {
      try {
    ...
    /*
     * At this point we have not found the class either in the cache or in this classloader, so
     * we need to ask the parent.
     */
