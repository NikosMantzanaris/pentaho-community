  ...
  public IPentahoResultSet executeQuery(Query queryObject ) {
    ...
    try {
      ...
      try {
        ...
        logger.error(Messages.getErrorString("SqlMetadataQueryExec.ERROR_0002_ERROR_EXECUTING_QUERY", e.getLocalizedMessage(), sql)); //$NON-NLS-1$
  ...
  protected SQLConnection getConnection(DatabaseMeta databaseMeta) {
    ...
    try {
      ...
      if (localConnection == null) {
        ...
        //Creating connections in PEC generate GenericDatabaseMeta objects that lack the DatabaseName (since GenericDatabaseMeta use a "custom URL" instead).
        //Later on when the db dialect of the database meta gets changed (this.getActiveDatabaseMeta()) to other than the "Generic" the 
