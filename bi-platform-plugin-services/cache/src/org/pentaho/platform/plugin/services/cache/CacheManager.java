    ...
    if (cacheEnabled) {
      ...
      if(cache != null) {
        try {
          ...
          CacheManager.logger.error(Messages.getString("CacheManager.ERROR_0006_CACHE_EXCEPTION", e.getLocalizedMessage())); //$NON-NLS-1$
        }
      } else {
  ...
  private LastModifiedCache buildCache(String key, Properties cacheProperties) {
    if(getCacheProvider() != null) {
      ...
      if(cacheExpirationRegistry != null) {
        ...
        logger.warn(Messages.getErrorString("CacheManager.WARN_0003_NO_CACHE_EXPIRATION_REGISTRY"));
      }
      return lmCache;
    } else {
      logger.error(Messages.getErrorString("CacheManager.ERROR_0004_CACHE_PROVIDER_NOT_AVAILABLE"));
      return null;
    }
  }
